...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use(session({
    ...
    cookie: { secure: false, httpOnly: true, sameSite: "strict", maxAge: 7 * 24 * 60 * 60 * 1000 }
  }));
...
// GET /api/admin/submissions/:id/file/:type?ffl=&created=
  ...
  app.get("/api/dealer/profile", async (req, res) => {
    try {
      ...
      return res.json({
        ...
        data: {
          id: d.id,
          tradeName: d.business_name,
          licenseName: d.contact_name,
          email: d.email,
          phone: d.phone,
          premiseAddress1: d.business_address,
          premiseCity: d.city,
          premiseState: d.state,
          premiseZipCode: d.zip,
          ein: d.ein,
          einType: d.ein_type,
    ...
    try {
      ...
      if (dealer.rows.length > 0 && dealer.rows[0].verified) {
        return res.json({
          ok: true,
          valid: true,
          tradeName: dealer.rows[0].business_name,
          licenseName: dealer.rows[0].contact_name || "",
          premiseAddress1: dealer.rows[0].business_address || "",
          premiseCity: dealer.rows[0].city || "",
          premiseState: dealer.rows[0].state || "",
          premiseZipCode: dealer.rows[0].zip || "",
          fflExpiry: dealer.rows[0].ffl_expiry_date || "",
          ein: dealer.rows[0].ein || "",
          einType: dealer.rows[0].ein_type || "",
        });
      }
  ...
  app.post("/api/ffl/upload", publicFormLimiter, async (req, res) => {
    try {
      ...
      // Database flags only (NO FastBound for pending dealers)
